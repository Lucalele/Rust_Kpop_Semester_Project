...
use Rust_Kpop_Semester_Project::companies::{insert_company, insert_label};
...
fn last_insert_id(connection: &mut SqliteConnection) -> QueryResult<i32> {
    let result: IdResult =
        diesel::sql_query("SELECT last_insert_rowid() AS id;").get_result(connection)?;

    i32::try_from(result.id)
...
fn table_count(connection: &mut SqliteConnection, table_name: &str) -> QueryResult<i64> {
    let escaped_name = table_name.replace('"', "\"\"");

    let query = format!("SELECT COUNT(*) AS count FROM \"{}\";", escaped_name);

    let result: CountResult = diesel::sql_query(query).get_result(connection)?;
...
fn database_zero_is_valid_and_all_operations_work() {
    ...
    let actual_table_names: Vec<&str> = tables.iter().map(|table| table.name.as_str()).collect();
    ...
    for table in &tables {
        let count = table_count(&mut connection, &table.name).unwrap_or_else(|error| {
    ...
    connection.test_transaction::<_, diesel::result::Error, _>(|connection| {
        ...
        let companies_before = table_count(connection, "companies")?;

        let labels_before = table_count(connection, "labels")?;

        let groups_before = table_count(connection, "idol_groups")?;

        let subunits_before = table_count(connection, "subunits")?;

        let project_groups_before = table_count(connection, "project_groups")?;

        let idols_before = table_count(connection, "idols")?;

        let idol_names_before = table_count(connection, "idol_names")?;

        let albums_before = table_count(connection, "albums")?;

        // -----------------------------------------------------------------
        // Company
        // -----------------------------------------------------------------

        insert_company(connection, "Database Zero Test Entertainment")?;
        ...
        insert_label(connection, "Database Zero Test Label", company_id)?;
        ...
        let project_group_id = last_insert_id(connection)?;
        ...
        insert_group_company(connection, group_id, company_id)?;
        ...
        insert_group_label(connection, group_id, label_id)?;
        ...
        insert_project_group_parent(connection, project_group_id, group_id)?;

        assert_eq!(
            table_count(connection, "project_group_parents")?,
            1,
        ...
        insert_idol(connection, "Female", Some(false))?;
        ...
        insert_idol_name(connection, member_id, "Database Zero Test Member")?;
        ...
        insert_idol(connection, "Male", Some(true))?;
        ...
        insert_idol_name(connection, soloist_id, "Database Zero Test Soloist")?;
        ...
        insert_idol_group_membership(connection, member_id, group_id)?;

        assert_eq!(
            table_count(connection, "idol_group_memberships")?,
            1,
            "Group membership was not inserted"
        );

        insert_idol_subunit_membership(connection, member_id, subunit_id)?;

        assert_eq!(
            table_count(connection, "idol_subunit_memberships")?,
            1,
            "Subunit membership was not inserted"
        );

        insert_idol_project_group_membership(connection, member_id, project_group_id)?;

        assert_eq!(
            table_count(connection, "idol_project_group_memberships")?,
            1,
        ...
        insert_idol_company(connection, soloist_id, company_id)?;
        ...
        insert_idol_label(connection, soloist_id, label_id)?;
        ...
        assert_eq!(
            violations_after_inserts.count, 0,
        ...
        let random_results = random_matching(connection, &no_filters, 2)?;
        ...
        assert!(
            limited_results
                .iter()
                .all(|album| { album.version.as_deref() == Some("Limited") }),
        ...
        assert!(
            soloist_results
                .iter()
                .all(|album| { album.artist_type == "Soloist" }),
        ...
        let group_name_results = random_matching(
            connection,
            &RandomizerFilters {
                artist_name: Some("Database Zero Test Group".to_string()),
                ..Default::default()
            },
            100,
        )?;
        ...
        let member_results = random_matching(
            connection,
            &RandomizerFilters {
                member_name: Some("Database Zero Test Member".to_string()),
                ..Default::default()
            },
            100,
        )?;
        ...
        let company_results = random_matching(
            connection,
            &RandomizerFilters {
                company_name: Some("Database Zero Test Entertainment".to_string()),
                ..Default::default()
            },
            100,
        )?;
        ...
        let label_results = random_matching(
            connection,
            &RandomizerFilters {
                label_name: Some("Database Zero Test Label".to_string()),
                ..Default::default()
            },
            100,
        )?;
        ...
        let empty_results = random_matching(
            connection,
            &RandomizerFilters {
                artist_name: Some("This Artist Does Not Exist".to_string()),
                ..Default::default()
            },
            100,
        )?;
